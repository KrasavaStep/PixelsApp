...
//    val pagedPhotos = viewModel.pagingPhotFlow.collectAsLazyPagingItems()
...
//    viewModel.handleIntent(HomeIntent.LoadInitialData())
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        LazyVerticalStaggeredGrid(
            ...
            items(
                count = pagedPhotos.itemCount,
                key = { index -> pagedPhotos[index]?.id ?: index }
            ) { index ->
                val item = pagedPhotos[index]
                if (item != null) {
                    PhotoCard(item) { id ->
